...
func (d *UserRepository) GetUserByEmailPassword(email, password string) (model.User, error) {
	var user model.User
	err := d.DB.Where("email = ? AND password = ?", email, password).Find(&user).Error
	if err != nil {
		return model.User{}, fmt.Errorf("we could not find the user with email => %s. Reason is %s", email, err)
	}
	return user, nil
}
